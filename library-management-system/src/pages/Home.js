...
function Home({ setRole, setAuthenticated }) {
  ...
  const handleLogin = async () => {
    ...
    if (response.data.result.status == true) {
      ...
      console.log("decoded token is ---", curr);
      setRole(curr?.role);
  ...
  return (
    <div className="  w-[100%]  p-10 box-border flex justify-between overflow-hidden h-[100vh]  items-center bg-gray-200">
      <div className="flex justify-center items-center">
        <div className="w-1/2 px-10  box-border ">
          <img
        ...
        <div className="w-[50%] h-[88vh] rounded-xl  bg-[#3B3B41] flex justify-center items-center flex-col">
          ...
          <label className="text-white">
            Welcome back! Please begin to your account{" "}
          </label>
          <div className="flex justify-center items-center">
            <input
              type="text"
          ...
          <button
            onClick={handleLogin}
            className="bg-blue-300 p-3 px-5 mt-4 rounded-full"
          >
            Login
          </button>
          ...
          <label
            className="bg-white text-black p-2 rounded-full mt-2"

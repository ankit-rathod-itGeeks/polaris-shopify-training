...
function ConfirmBox({ setToSubmit, bookToSubmit,setSelectSubmit, setToGetUserId }) {
    ...
    const handleSubmit = async () => {
        ...
        axios.delete('http://localhost:8000/admin/submitBook', { data: { userId: bookToSubmit.userId, bookId: bookToSubmit.bookId ,penalty:penalty,returnedDate:getTodayDate()} }).then((res) => {
            // if()
    ...
    return (
        <div className='mt-4 flex justify-center h-[100%] items-center '>
            <div className='bg-[#3B3B41] font-bold  h-[100%] text-white  rounded-lg p-4 text-xl w-[50%] '>
                ...
                <div className=''> <label>Returning Date</label> : {getTodayDate()}</div>
                <div className=''> <label>Penalty</label> : Rs. {penalty}</div>
